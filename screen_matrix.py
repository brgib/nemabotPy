...
def draw(sim, surface, rect):
    ...
    if num_neurons == 0:
        sim.draw_text(surface, "Aucun neurone.", 10, 10); return
    ...
    neuron_idx = 0; neuron_positions = {}
    for i in range(grid_size):
        for j in range(grid_size):
            if neuron_idx >= num_neurons: break
            ...
            if cell_rect.collidepoint(pos):
                if button == 1:
                    ...
                    if neuron_name in sim.neuron_data:
                        del sim.neuron_data[neuron_name]; sim.scale_reset = True
                    else:
